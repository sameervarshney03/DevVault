...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || "Login failed. Is the backend server running?");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] flex flex-col items-center justify-center px-4">
      ...
      <div className="mb-8 flex flex-col items-center gap-3">
        <img src="/devvault.svg" alt="DevVault Logo" className="w-16 h-16" />
        <h1 className="text-2xl font-bold text-white">Sign in to DevVault</h1>
      </div>
